...
@register_loss("shape4classify_finetune_cross_entropy")
class Shape4classifyFinetuneCrossEntropyLoss(CrossEntropyLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        net_output = model(
            **sample["net_input"],
            ...
            cloudNet_name=self.args.cloudNet_name,
        )
...
@register_loss("shape4classify_mse")
class Shape4classifyMSELoss(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
...
@register_loss("shape4classify")
class Shape4classifyLoss(Shape4classifyMSELoss):
    def __init__(self, task):
        super().__init__(task)
...
@register_loss("shape4classify_finetune_multi_task_BCE")
class Shape4classifyFinetuneMultiTaskBCELoss(CrossEntropyLoss):
    def __init__(self, task):
        super().__init__(task)
