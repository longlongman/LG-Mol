...
class Encoder_point_cloud(nn.Module):
    ...
    def forward(self, batch_size, x, edge_index, edge_attr, pos, points, points_atom_index, x_library_fragment_index, x_subgraph, subgraph_edge_index, subgraph_edge_attr, subgraph_pos, subgraph_points, subgraph_points_atom_index, x_subgraph_library_fragment_index, query_indices, query_indices_batch, fragment_batch, device = torch.device('cpu')):
        ...
        fragment_library_features, fragment_library_node_features, fragment_library_batch = self.encode_fragment_library(fragment_batch, device = device) # 使用 gnn 对 fragment 进行编码

        x = torch.cat((x, fragment_library_features[x_library_fragment_index]), dim = 1) # x - node representation, x_library_fragment_index - x 所属的 fragment 类型
        
        x_subgraph = torch.cat((x_subgraph, fragment_library_features[x_subgraph_library_fragment_index]), dim = 1) # x_subgraph - partial graph node representation
        
        _, Z_equi, Z_inv, _, _, h_mean, h_std, h_reshaped_gnn, h_predicted_reshaped, h_reshaped = self.encode(
            x,
            edge_index, 
            pos, # 置中后的坐标数据
            points, # 点云数据，每个 atom 周围高斯采样 5 points
            points_atom_index, 
            edge_attr, 
            batch_size,
            select_indices = None,
