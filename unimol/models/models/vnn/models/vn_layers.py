...
class VNLinearLeakyReLU(nn.Module):
    # def __init__(self, in_channels, out_channels, dim=5, share_nonlinearity=False, negative_slope=0.2, use_batchnorm=False):
    ...
    def forward(self, x):
        ...
        # Linear
        # x = x.to(torch.half)
        p = self.map_to_feat(x.transpose(1,-1)).transpose(1,-1) # 等变的
...
class VNStdFeature(nn.Module):
    ...
    def forward(self, x):
        ...
        z0 = self.vn1(z0)
        z0 = self.vn2(z0)
        # z0 = self.vn_lin(z0.to(torch.float16).transpose(1, -1)).transpose(1, -1) # this is just a VNNLinear layer, without any activation
        z0 = self.vn_lin(z0.transpose(1, -1)).transpose(1, -1) # this is just a VNNLinear layer, without any activation
        ...
        if self.dim == 4:
            # x_std = torch.einsum('bijm,bjkm->bikm', x.to(torch.half), z0)
