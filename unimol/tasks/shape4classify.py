...
from unimol.data.shape4classify_dataset import CloudDataset
from unimol.data.cloud_pad_dataset import RightPadDatasetCloud
...
@register_task("shape4classify")
class Shape4classify(UnicoreTask):
    ...
    def load_dataset(self, split, **kwargs):
        ...
        # logger.info("------------------------------")
        logger.info(f"Points Num: {self.args.points_num}")
        # logger.info("------------------------------")
        if split == "train":
            tgt_dataset = KeyDataset(dataset, "target")
            smi_dataset = KeyDataset(dataset, "smi")
            cloud_dataset = KeyDataset(dataset, "charge")
        ...
        cloud_dataset = FromNumpyDataset(cloud_dataset)
        cloud_dataset = CloudDataset(cloud_dataset, self.args.points_num)
        ...
        nest_dataset = NestedDictionaryDataset(
            {
                "net_input": {
                    ...
                    "src_cloud": RightPadDatasetCloud(
                        cloud_dataset,
                        pad_idx=0,
                    ),
                },
    ...
    def build_model(self, args):
        ...
        model.register_cloud_feature(
            self.args.cloudNet_name,
        )

        model.register_mergeclassification_head(
